...
def zero_shot_eval(model, data, epoch, args):
    ...
    if 'zeroshot' in data:
        classnames = data['zeroshot'].dataloader.dataset.classes
        #cifar_templates = [ lambda c: f"a photo of a {c}."]
    ...
    if 'zeroshot' in data:
        top1, top5 = run(model, classifier, data['zeroshot'].dataloader, args, 'zeroshot')
